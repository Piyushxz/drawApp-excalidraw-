'use client'
import { HomeIcon, LogOut, MenuIcon, Monitor, Moon, Sun, Trash, Trash2, Users, Edit3 } from "lucide-react"
...
export const Menu = ({
    ...
    setShowLiveCollaborationModal,
    hasSelectedShape = false,
    onEditClick
}: {
    game: Game,
    setShowClearCanvasModal: Dispatch<SetStateAction<boolean>>,
    setShowLiveCollaborationModal: Dispatch<SetStateAction<boolean>>,
    ...
    return(
        <div className="fixed bottom-4 right-4 z-50 md:hidden">
            ...
            <DropdownMenu>
                ...
                <DropdownMenuContent collisionPadding={{right:12,bottom:15}} className="w-48 bg-[#191919] border border-white/15">
                <DropdownMenuItem className="w-full justify-between flex items-center gap-2 w-full  rounded-sm px-3 py-1.5 text-md outline-none text-white  rounded-lg transition-all ease-in-out cursor-pointer">
                    <span className=" font-normal text-md  tracking-tighter text-inherit ">Theme</span>
                        <div className="flex rounded-xl gap-2 border border-white/15 px-3 py-1">
                            <button title="Light Mode" onClick={(e)=>{
            ...
            {/* Edit Icon - Only show when shape is selected on mobile */}
            {hasSelectedShape && onEditClick && (
                <div className="fixed bottom-4 right-4 translate-x-[-60px] bg-[#191919] rounded-lg shadow-lg p-2 border border-gray-600/30">
                    <Edit3 
                        size={20} 
